...
(function($, window) {
  ...
  Sammy.Application = function(app_function) {
    ...
    this.listener_mappings = new Sammy.Object({});
    this.listener_uids     = [];
    this.arounds           = [];
    this.befores           = [];
  ...
  Sammy.Application.prototype = $.extend({}, Sammy.Object.prototype, {
    ...
    //
    bind: function(name, data, callback, store_callback_pointer) {
      var app = this;
      // build the callback
      ...
      if(store_callback_pointer) {
          this.listener_uids.push(uid);
          this.listener_mappings[uid] = this.listeners[name].length - 1;
      } 
    ...
    // Works just like `jQuery.fn.unbind`. Takes the user initiated callback
    // function as a parameter.
    //
    // For example:
    // var uid;
    // this.bind('test', undefined, function() {
    //   alert('one time!');
    //   this.unbind('test', uid);
    // }, true);
    // uid = this.listener_uids.pop();
    // 
    unbind: function(name, uid) {
        var index = this.listener_mappings[uid]; 
        if(index >= 0) {
            var listener = this.listeners[name][index];
            this._unlisten(name, listener);
            this.listeners[name].splice( index, 1 );
            delete this.listener_mappings[uid];
        } else {
    ...
    // Binds a event listener that fires only once
    bindOneShot: function(name, callback) {
        var self = this;
        var fired = false;
        var uid = this.bind(name, undefined, function() {
            try {
                self.unbind(name, uid);
                if($.isFunction(callback)) {
                    callback.apply(this, arguments);
                }
            } catch(e) { console.log("'"+name+"' for "+uid+": "+e); }   
        }, true).listener_uids.pop();
        return this;
    },
    ...
    run: function(start_url) {
      ...
      this.bind('submit', this._handleFormSubmission);
      ...
      // trigger post run for those who need to attach event handlers
      // dynamically after starting
    ...
    _handleFormSubmission: function(e) {
        var returned = this.app._checkFormSubmission($(e.target).closest('form'));
    ...
    unload: function() {
      ...
      // unbind form submits
      this.$element().unbind('submit', this._handleFormSubmission).removeClass(app.eventNamespace());
      // unbind all events
